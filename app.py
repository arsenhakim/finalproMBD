...
if page == "Edit Data":
    with conn.session as session:
        if st.button('Tambah Data'):
            with conn.session as session:
                query = text('INSERT INTO produk (product_name, category, brand, price, discounted, ratings_1_to_5, supplier_name) VALUES (:product_name, :category, :brand, :price, :discounted, :ratings_1_to_5, :supplier_name);')
                session.execute(query, {'product_name': '', 'category': '', 'brand': '', 'price': '', 'discounted': '', 'ratings_1_to_5': None, 'supplier_name': ''})
                session.commit()
